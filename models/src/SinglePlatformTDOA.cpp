...
// SinglePlatformTDOA.h 已经包含了 InterferometerPositioning.h
...
// 验证SNR是否满足要求
bool SinglePlatformTDOA::validateSNR(const ReconnaissanceDevice& device, 
...
// 验证角度是否在辐射源工作扇区范围内
bool SinglePlatformTDOA::validateAngle(const RadiationSource& source,
                                     double azimuth,
                                     double elevation) {
    // 获取辐射源工作扇区范围
...
LocationResult SinglePlatformTDOA::runSimulation(const ReconnaissanceDevice& device, 
    ...
    // 检查SNR验证 - 添加SNR计算和验证
    bool snrValid = validateSNR(device, source, distance1);
    if (!snrValid) {
        g_print("警告: SNR验证失败，但继续进行定位计算\n");
        // 记录SNR验证结果到LocationResult，但不中断计算
        result.validSNR = false;
    } else {
        result.validSNR = true;
    }
    ...
    // 检查角度是否在辐射源的工作扇区范围内
    bool angleValid = validateAngle(source, azimuth, elevation);
    if (!angleValid) {
        g_print("警告: 角度验证失败，已进行调整\n");
        
        // 记录角度验证结果到LocationResult
        result.validAngle = false;
    } else {
        result.validAngle = true;
    }
    ...
    // 输出验证结果的摘要
    if (!result.validAngle && !result.validSNR) {
        g_print("警告: 角度验证和SNR验证均失败，结果可能不准确\n");
    } else if (!result.validAngle) {
        g_print("警告: 角度验证失败，辐射源可能不在工作扇区内或方向计算有误\n");
    } else if (!result.validSNR) {
        g_print("警告: SNR验证失败，设备与辐射源距离过远或信号强度不足\n");
    } else {
        g_print("验证结果: 所有验证均通过，结果可靠\n");
    }
