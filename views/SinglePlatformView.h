...
extern "C" {
	...
	// 新增：测向误差线控制按钮回调函数
	void onShowErrorLinesCallback(GtkWidget* widget, gpointer data);
	void onClearErrorLinesCallback(GtkWidget* widget, gpointer data);
}
...
public:
	...
	// 定位算法变化回调
	static void onAlgorithmChanged(GtkWidget* widget, gpointer data);
	...
	// 更新精度分析表格
	void updateErrorTable(const std::string& positioningAlgorithm);
	...
	// 获取选中的定位算法
	std::string getSelectedPositioningAlgorithm() const;

	// 新增：获取选中的技术体制（返回内部编码，如"INTERFEROMETER"/"TDOA"）
...
private:
	...
	// 新增：测向误差信息显示控件
	GtkWidget* m_directionErrorValue;
	GtkWidget* m_techInfoValue;
